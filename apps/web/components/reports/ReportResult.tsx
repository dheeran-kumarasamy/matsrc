...
function MaterialConsumptionResult({ rows }: { rows: MaterialConsumptionRow[] }) {
  if (rows.length === 0) {
    return <p className="text-xs font-semibold text-slate-400">No order history yet — place an order to see this report.</p>;
  }
...
function CostSavingsResult({ summary }: { summary: CostSavingsSummary }) {
  if (summary.rows.length === 0) {
    return <p className="text-xs font-semibold text-slate-400">No potential savings found right now — you&apos;re already getting the best price on your usual materials.</p>;
  }
      ...
      <div className="max-h-56 overflow-y-auto rounded-lg border border-slate-100">
        <table className="min-w-full text-xs">
          <thead className="bg-slate-50 text-left">
            <tr>
              <th className="px-3 py-2.5 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Material</th>
              <th className="px-3 py-2.5 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Paid</th>
              <th className="px-3 py-2.5 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Best available</th>
              <th className="px-3 py-2.5 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Savings</th>
            </tr>
          </thead>
          <tbody>
            {summary.rows.map((row) => (
              <tr key={row.productId} className="border-t border-slate-100">
                <td className="px-3 py-2.5 font-semibold text-slate-800">{row.name}</td>
                <td className="px-3 py-2.5 font-medium text-slate-600">₹{row.amountPaid.toLocaleString("en-IN")}</td>
                <td className="px-3 py-2.5 font-medium text-slate-600">
                  ₹{(row.currentBestUnitPrice * row.quantityOrdered).toLocaleString("en-IN")}
                </td>
                <td className="report-display px-3 py-2.5 text-base text-emerald-700">
